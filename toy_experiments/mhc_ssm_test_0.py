...
CONFIG = {
    "D_MODEL": 256,       # Smaller dimension for stability
    "N_LAYERS": 4,        # Fewer layers for faster debugging
    "VOCAB": 64,          # Sanity Check Vocab
    "STREAMS": 4,         
    "BS": 32,
    "SEQ": 256,            # Short sequence
    "STEPS": 8000,        
    "LR": 1e-3
}

# ==========================================
# 1. Corrected Mamba-2 Block (Pre-Norm + Sticky Init)
# ==========================================
class Mamba2SimpleBlock(nn.Module):
    def __init__(self, d_model, d_state=64, expand=2):
        ...
        self.n_heads = 4 # Fixed heads for simplicity
        
        # 1. Input Projection
        # Projects to: z (gate), x (value), B (state), C (state), dt (time)
        # Dimensions:
        # z: d_inner
        # x: d_inner
        # B: n_heads * d_state
        ...
        # "Sticky" Initialization: A starts very small so state doesn't decay
        self.A_log = nn.Parameter(torch.ones(self.n_heads) * -4.0) 
        self.D = nn.Parameter(torch.ones(self.n_heads))
        self.dt_bias = nn.Parameter(torch.rand(self.n_heads) - 4.0) # Bias towards small dt
        
        self.out_proj = nn.Linear(self.d_inner, d_model)
        self.norm = nn.RMSNorm(d_model) # Pre-Norm

    def forward(self, u):
        # PRE-NORM: Apply norm BEFORE projection
        u_norm = self.norm(u)
        ...
        # x: [B, L, D_inner]
        # dt: [B, L, n_heads]
        # B_mat: [B, L, n_heads, d_state]
        # C_mat: [B, L, n_heads, d_state]
        ...
        # Reshape x for multi-head recurrence
        x = x.view(B_size, L, n_heads, -1) # [B, L, H, head_dim]
        
        # Activation for dt
        dt = F.softplus(dt + self.dt_bias) # [B, L, H]
        
        # Discretize A: exp(dt * A)
        A = -torch.exp(self.A_log) # Negative A
        dtA = torch.exp(dt * A.view(1, 1, -1)) # [B, L, H]
        
        # Discretize B: dt * B
        dtB = torch.einsum('blh, blhn -> blhn', dt, B_mat) # [B, L, H, N]
        
        # --- Recurrence Scan (The "SSM" Core) ---
        # h_t = h_{t-1} * dtA_t + dtB_t * x_t
        # y_t = C_t * h_t
        ...
        for t in range(L):
            dtA_t = dtA[:, t, :, None, None] # [B, H, 1, 1]
            dtB_t = dtB[:, t, :, :, None]    # [B, H, N, 1]
            x_t = x[:, t, :, None, :]         # [B, H, 1, P]
            C_t = C_mat[:, t, :, None, :]     # [B, H, 1, N]
            
            # State update
            h = h * dtA_t + torch.matmul(dtB_t, x_t)
            
            # Output
            y_t = torch.matmul(C_t, h).squeeze(-2) # [B, H, P]
            ys.append(y_t)
...
class MHC_Connection(nn.Module):
    def __init__(self, d_model, n_streams=4):
        ...
        # Simple learnable router
        self.W_pre = nn.Parameter(torch.randn(n_streams, 1) * 0.01)

    def forward(self, H, layer_fn):
        # H: [B, L, N, D_sub]
        
        # 1. Mixing (Identity Path)
        M = self.mixer()
        H_res = torch.einsum('bsid, ji -> bsjd', H, M)
        
        # 2. Layer Path
        # Aggregate streams -> Flatten
        # Simple average weighted by learnable param for now
        # [B, L, N, D] -> [B, L, D_model]
        x_flat = H.flatten(2, 3) 
        
        # Pass through Layer
        x_out = layer_fn(x_flat) # [B, L, D_model]
...
class Model_Standard(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            # Residual is handled here
            x = x + layer(x)
        return self.head(self.norm(x))

class Model_mHC(nn.Module):
    def __init__(self, d_model, n_layers, vocab_size, n_streams=4):
        super().__init__()
        self.n_streams = n_streams
        self.d_sub = d_model // n_streams
        self.embed = nn.Embedding(vocab_size, d_model)
...
def run_experiment(model_type, run_id):
    ...
    if model_type == "standard":
        ...
        model = Model_mHC(CONFIG["D_MODEL"], CONFIG["N_LAYERS"], CONFIG["VOCAB"], CONFIG["STREAMS"]).cuda()
    ...
    for i in range(CONFIG["STEPS"]):
        ...
        # --- MASKING FIX ---
        # Only calculate loss on the second half (the copy part)
        # We want to see if it remembers the past, not if it can guess random future numbers.
        split_idx = CONFIG["SEQ"] // 2
        
        valid_logits = logits[:, split_idx-1 : -1, :] # Align prediction to target
        valid_y = y[:, split_idx-1 : -1]
        
        # Flatten for CrossEntropy
        loss = loss_fn(valid_logits.reshape(-1, CONFIG["VOCAB"]), valid_y.reshape(-1))
        
        # Calculate Accuracy on the copy part only
...
if __name__ == "__main__":
    run_id = datetime.datetime.now().strftime("exp_%Y%m%d_%H%M%S")
    print(f"ID: {run_id} | Config: {CONFIG}")
    std_h, _ = run_experiment("standard", run_id)
    mhc_h, _ = run_experiment("mhc", run_id)
    log_experiment(run_id, std_h, mhc_h)